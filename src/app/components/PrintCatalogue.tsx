...
export function PrintCatalogue({
  products,
  title = 'Estate Collection Catalogue v2.0',
  showPriceList = true,
    ...
    <div className="print-catalogue bg-white">
      ...
      <div className="page cover-page relative min-h-screen overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-black/20 to-black/40 cover-overlay"></div>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-12 text-white">
          <div className="text-center max-w-3xl">
            ...
            <div className="flex items-center justify-center gap-8 text-white/80">
              ...
              <div className="text-center">
                <p className="text-4xl font-light mb-1">{getSortedCategories(products).length}</p>
                <p className="text-sm uppercase tracking-wider">Categories</p>
              </div>
              <div className="w-px h-12 bg-white/30"></div>
              <div className="text-center">
                <p className="text-lg font-light mb-1">{new Date().toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
        ...
        <div className="absolute bottom-12 left-0 right-0 text-center">
          <div className="inline-block px-8 py-3 border border-white/30 backdrop-blur-sm mb-4">
            <p className="text-white/70 text-sm tracking-wide">Carefully curated • Authentically sourced • Expertly catalogued</p>
          </div>

          {/* Contact Information */}
          <div className="mb-3">
            <p className="text-white/90 text-sm mb-2 font-medium">To arrange viewing and for further enquiries:</p>
      ...
      <div className="page min-h-screen p-12 break-before-page bg-gradient-to-br from-white to-gray-50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid gap-4">
            {getSortedCategories(products).map((category, idx) => {
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          <span className="font-medium">${Math.round(minPrice).toLocaleString()}</span>
                        </span>
                        <span className="text-gray-300">•</span>
                        <span className="flex items-center gap-1">
      ...
      {getSortedCategories(products).map((category, categoryIndex) => {
        const categoryProducts = sortProductsById(products.filter(p => p.category === category));
                ...
                <div className="text-center max-w-2xl">
                  ...
                  <div className="bg-white/60 backdrop-blur-sm p-6 rounded-lg border border-gray-200">
                    <p className="text-gray-700 leading-relaxed">
                      {categoryProducts.length} curated items from ${Math.round(Math.min(...categoryProducts.map(p => p.price))).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
                        ...
                        >
                          ...
                          <div className="aspect-square bg-gray-50 flex items-center justify-center overflow-hidden">
                            <CompressedImage
                              src={product.image}
                              alt={product.name}
                              className="w-full h-full object-contain object-center catalog-product-image"
                              maxWidth={1200}
                              maxHeight={1200}
                              quality={0.85}
                            />
                          ...
                          <div className="p-4">
                            ...
                            {/* Smaller price */}
                            <div className="mb-3">
                              <p className="text-lg font-bold text-gray-900"><span className="font-normal text-sm">Asking Price:</span> ${Math.round(product.price).toLocaleString()}</p>
                            </div>
      ...
      <div className="page min-h-screen p-12 break-before-page bg-gradient-to-br from-white to-gray-50">
        ...
        {getSortedCategories(products).map((category, catIdx) => {
          const categoryProducts = sortProductsById(products.filter(p => p.category === category));
            ...
            <div key={category} className={catIdx > 0 ? 'mt-8' : ''}>
                ...
                <div className="text-sm">
                  ...
                  {showCategoryTotals && (
                    <>
                      ...
                      <span className="font-medium">Total: ${Math.round(categoryTotal).toLocaleString()}</span>
                    </>
                  )}
                </div>
              </div>
              ...
              {categoryProducts.map((product, idx) => (
                ...
                >
                  ...
                  <div className="text-lg font-semibold text-gray-900">${Math.round(product.price).toLocaleString()}</div>
        ...
        <div className="mt-8 bg-gray-900 text-white px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg">{products.length} items across {getSortedCategories(products).length} categories</p>
          </div>
          <div className="text-right">
            <p className="text-sm uppercase tracking-wider opacity-80 mb-1">Total Value</p>
            <p className="text-3xl font-light">${Math.round(products.reduce((sum, p) => sum + p.price, 0)).toLocaleString()}</p>
          </div>
        </div>
        )}
      </div>
      ...
      {/* Part II Teaser Page */}
      <div className="page min-h-screen break-before-page relative overflow-hidden">
        {/* Full-bleed background image - brightened */}
        <div className="absolute inset-0">
          <CompressedImage
            src={houswaresImage}
            alt="Living and Dining Housewares"
            className="w-full h-full object-cover teaser-background-image"
            maxWidth={1920}
            maxHeight={1920}
            quality={0.85}
            style={{ objectPosition: 'center' }}
          />
          <div className="absolute inset-0 bg-gradient-to-r from-white/95 via-white/60 to-transparent teaser-overlay"></div>
        </div>

        {/* Content anchored left with white background panel */}
        <div className="relative z-10 flex flex-col justify-center min-h-screen p-16">
          <div className="max-w-xl bg-white/90 backdrop-blur-sm p-10 rounded-lg shadow-lg">
            <div className="mb-6">
              <div className="inline-block px-5 py-2 border border-gray-300 rounded-full mb-8">
                <p className="text-gray-600 text-xs uppercase tracking-widest">Coming Next</p>
              </div>
            </div>

            <p className="text-gray-500 text-sm uppercase tracking-widest mb-4 font-medium">Estate Collection</p>

            <h2 className="text-6xl font-serif text-gray-900 leading-tight mb-4">
              Part II
            </h2>

            <div className="w-20 h-1 bg-gray-800 mb-8"></div>

            <h3 className="text-3xl font-serif text-gray-800 leading-snug mb-6">
              Housewares
            </h3>
          ...
          <div className="max-w-2xl">
            <div className="mb-12">
              ...
              <p className="text-xl text-white/80 leading-relaxed">
                ...
                across {getSortedCategories(products).length} categories.
              </p>
            </div>
      ...
      <style>{`
        ...
        /* Brighten cover image for print */
        .print-catalogue .cover-image {
          filter: brightness(1.2) contrast(1.1);
        }

        .print-catalogue .cover-overlay {
          background: linear-gradient(to bottom, rgba(0, 0, 0, 0.25), rgba(0, 0, 0, 0.15), rgba(0, 0, 0, 0.35)) !important;
        }
        ...
        /* Enhanced product images - show full image, enhance quality */
        .print-catalogue .catalog-product-image {
          object-fit: contain;
          object-position: center;
        ...
        .print-catalogue .teaser-background-image {
          filter: brightness(0.95) contrast(1.1);
        }

        .print-catalogue .teaser-overlay {
          background: linear-gradient(to right, rgba(255, 255, 255, 0.95), rgba(255, 255, 255, 0.6), transparent) !important;
        }
        ...
        @media print {
          ...
          * {
          ...
          /* Enhanced product images for print - extra brightness and sharpness */
          .print-catalogue .catalog-product-image {
            object-fit: contain !important;
          ...
          /* Brighten and enhance cover image for print */
          .print-catalogue .cover-image {
          ...
          /* Lighter overlay for print to show more of the image */
          .print-catalogue .cover-overlay {
            background: linear-gradient(to bottom, rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.1), rgba(0, 0, 0, 0.3)) !important;
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
          }

          /* Teaser page for print */
          .print-catalogue .teaser-background-image {
            filter: brightness(1.0) contrast(1.15) !important;
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
          }
